...
import GMap from '../components/GMap.jsx'; import MapMock from '../components/MapMock.jsx';
import { uploadDraft } from '../lib/api.js';

export default function MapCreateUI({ draft, setDraft, onBack, onPostSuccess, onPostFail }){
  ...
  async function handlePost(){
    try{
      const resp = await uploadDraft(draft);
      onPostSuccess?.(resp.map_id);
    }catch(e){
      console.error(e);
      onPostFail?.();
    }
  ...
  return (<section className="card">
    ...
    <div className="row" style={{marginTop:12}}>
      <button className="btn" onClick={undo} disabled={!draft.path.length}>ひとつ戻す</button>
    </div>
    ...
    <div className="row" style={{marginTop:16}}>
      ...
      <button className="btn primary" onClick={handlePost} disabled={pointCount<2 || !draft.title}>投稿</button>
    </div>
  </section>);
}
